...
const FAQItem = ({ pregunta, respuesta }: { pregunta: string; respuesta: string }) => {
  ...
  return (
    <div className={`border rounded-xl overflow-hidden transition-all ${open ? 'border-[#0f49bd]' : 'border-gray-100'}`}>
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-5 py-4 text-left bg-white hover:bg-gray-50 transition-colors"
      >
        <span className="font-medium text-gray-900 text-sm pr-4">{pregunta}</span>
        <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-all ${open ? 'bg-[#0f49bd] text-white rotate-45' : 'bg-gray-100 text-gray-500'}`}>
        ...
        <div className="max-w-3xl mx-auto space-y-10">
          {faqs.map((grupo) => (
            <div key={grupo.categoria}>
              <h2 className="text-xs font-bold text-[#0f49bd] uppercase tracking-widest mb-4">
                {grupo.categoria}
              </h2>
          ...
          <div className="bg-gradient-to-br from-[#002f87] to-[#0f49bd] rounded-2xl p-8 text-center text-white">
